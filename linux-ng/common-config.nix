# Lifted mostly unmodified from nixpkgs/pkgs/os-specific/linux/kernel/common-config.nix
...
let
  ...
  defaultFeatures = {
    ...
    xen_dom0 = false;
    criu = false;
    criu_revert_expert = true;
  };

  features = defaultFeatures // _features;
  ...
  # flattenKConf = nested: mapAttrs (_: v: assert length v == 1; head v) (zipAttrs (attrValues nested));
  flattenKConf = nested: mapAttrs (_: head) (zipAttrs (attrValues nested));

  options = {

    debug = {
      DEBUG_INFO                = if features.debug then yes else no;
      DEBUG_KERNEL              = yes;
      DEBUG_DEVRES              = no;
      DYNAMIC_DEBUG             = yes;
    ...
    } // optionalAttrs features.criu ({
      EXPERT              = yes;
      CHECKPOINT_RESTORE  = yes;
    } // optionalAttrs features.criu_revert_expert {
